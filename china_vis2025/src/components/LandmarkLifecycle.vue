        ...
        <div v-for="(event, index) in sortedTimelineEvents" :key="index" class="timeline-item">
          <div class="timeline-marker">
            <div class="timeline-event-info">
              ...
              <img v-if="event.image" :src="getImagePath(event.image)" :alt="event.title" class="event-image-tooltip"/>
            </div>
          </div>
          <div class="timeline-item-date">{{ event.year || getYearFromDate(event.date) }}</div>
        </div>
      </div>
...
export default {
    ...
    return {
      ...
      allLifecycleData: {
        dashilar: [
          ...
          { year: 1420, date: "明永乐年间 (15世纪初)", title: "廊房商业街区兴建", description: "北京鼓励商贸，在皇城四门等地兴建廊房商业街区。", image: "olddashilan.jpg" },
          { year: 1488, date: "明弘治元年 (1488年)", title: "“大栅栏”得名", description: "为维护京师治安，廊房四条两端设木质栅栏，由商户出资修建，规模格外巨大，逐渐被称为“大栅栏”。", image: "dashilan_bg.jpg" },
          ...
          { year: 0, date: "风格演变", title: "建筑风格", description: "承袭北京传统商街风格，主要以灰砖灰瓦为主，辅以雕花木窗、红柱绿瓦等明清装饰元素。部分商号立面引入西洋彩色瓷砖等外来元素。街口的标志性铁艺拱门即为现代建构。", image: "dashilan_bg.jpg" },
        ],
        mingTombs: [
          // ... (明十三陵数据，同上次回复，确保year和image文件名正确)
          { year: 1409, date: "永乐七年 (1409年)", title: "长陵始建", description: "明代从永乐帝(朱棣)在此建长陵开始营建十三陵。", image: "长陵.jpg" },
          { year: 1644, date: "崇祯末年 (1644年)", title: "营建终止", description: "明十三陵的营建终止于崇祯帝入陵。", image: "" },
          ...
          { year: 1950, date: "1950年代", title: "考古发掘", description: "在吴晗、郭沫若等文史专家倡议下，先后发掘并开放了定陵和长陵等。", image: "碑亭.jpg" },
          ...
          { year: 1966, date: "文革期间 (1966年起)", title: "严重破坏", description: "十三陵遭到严重破坏，万历帝及皇后的遗骸被红卫兵烧毁，多处建筑被拆毁。", image: "" },
          { year: 1972, date: "1970年代后", title: "加强保护", description: "政府加强保护。定陵博物馆于1959年开馆，1972年归属北京园林局管理。", image: "" },
          ...
          { year: 0, date: "风格演变", title: "建筑风格", description: "沿袭古制，体现皇家陵寝规格。大型土木结构均采用黄琉璃瓦屋面和朱红梁柱。神道沿线遍布石像生。石牌坊建于嘉靖十九年(1540年)。21世纪的修缮工程也用了钢结构加固、仿古琉璃瓦复盖等现代技术。", image: "明十三陵神道_大宫门.jpg" },
        ],
      }
    };
  },
  computed: {
    ...
    currentSectionTitle() {
      if (this.landmarkId === "dashilar") {
        const titles = { lifeCycle: "街巷记忆", influence: "商韵流传", legends: "坊间拾趣"};
        return titles[this.direction] || "详情探索";
      } else if (this.landmarkId === "mingTombs") {
        const titles = { lifeCycle: "皇陵沿革", influence: "世遗之尊", legends: "陵寝秘语"};
        return titles[this.direction] || "详情探索";
      }
      return "生命年轮"; // Default
    },
    currentSectionIntro() {
      ...
      return (intros[this.landmarkId] && intros[this.landmarkId][this.direction]) || `探索${name}的更多详情。`;
    },
    landmarkNamePlaceholder(){ // 用于 intro text 中在 landmarkName 加载前提供占位
  ...
  methods: {
    goBackToPortal() {
      ...
      if (this.landmarkId === 'dashilar') {
        portalRouteName = 'DashilarPortal'; // 假设大栅栏门户页的路由名称
      } else if (this.landmarkId === 'mingTombs') {
        portalRouteName = 'MingTombsPortal'; // 假设明十三陵门户页的路由名称
      }
      // 检查路由是否存在，如果不存在则回退到更通用的页面或首页
      if (this.$router.hasRoute(portalRouteName)) {
        this.$router.push({ name: portalRouteName, params: { landmarkId: this.landmarkId } });
      } else {
        this.$router.push('/'); // 如果特定门户路由不存在，则返回首页
      }
    },
    loadLifecycleData() {
      this.landmarkName = this.landmarkId === "dashilar" ? "大栅栏" : "明十三陵";
      ...
      if (this.direction === 'lifeCycle') {
        ...
        console.log(`加载 ${this.landmarkName} 的 ${this.currentSectionTitle} 内容...`);
      }
    },
...
.timeline-section { /* For lifecycle */
  width: 100%;
  max-width: 1200px;
  margin: 20px auto;
  padding: 20px;
  ...
  &::before {
    left: 0;
    background: linear-gradient(to right, rgba(249, 245, 237, 0.9), transparent);
  }
...
.content-section { /* For other directions like influence, legends */
  width: 100%;
  max-width: 900px;
  margin: 20px auto;
  padding: 30px;
